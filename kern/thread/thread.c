...
thread_bootstrap(void)
{
	...
	/* Set curthread */
	curthread = me;
	if(process_addthread(sysproc, me)){
		panic("thread_bootstrap: Failed on attaching system process\n");
	}

	/* Activate sysproc vmspace. */
 ...
 * DATA1 = args, DATA2 = nargs, at most of time.
 * proc is parent process
 */
int
	    ...
	    struct thread **ret)
{
	...
	/* If proc is NULL, inherit the current process */
	if (proc == NULL){
		proc = curproc;
	}
	...
	// 	newguy->t_vmspace = proc->p_vmspace;
	// }

	result = process_addthread(proc, newguy);
	if (result) {
		goto fail;
	}
...
thread_exit(void)
{
	...
	if (curthread->t_vmspace) {
		 ...
		 */
		/*
		 * as will be destoryed when process exit.
		 */
...
/*
void
thread_sleep_wrap(const void *addr)
{

	int spl = splhigh(addr);
	thread_sleep;
	splx(spl);
 ...
 * ADDR. ONLY ONE THREAD IS WAKEN UP.
 */
