 ...
 * TLB Operations:
 *
 *   TLB_Clear: Clear everything in TLB for a new process. Invalid every
 *      entry
 *
 *   TLB_Get_Slot: Get a free slot in TLB. If TLB is full, call TLB_Evict()
 *		to throw a random victim.
 *
 *   TLB_Evict: Evict a random slot.
 *
 * 	 TLB_Invalid: Invalid a selected entry by index.
 *
 *	 TLB_Unmap: Invalid a TLB according to a virtual address if the vaddr can be found.
 *
 */
