	     ...
	     int is_executable)
{
	...
	result = VOP_READ(v, &u);
	if (result) {
		kprintf("load_segment: VOP_READ failed.\n");
		return result;
	}

	// if (u.uio_resid != 0) {
	// 	/* short read; problem with executable? */
	// 	kprintf("ELF: short read on segment - file truncated?\n");ELF: short read on segment - file truncated?
	// 	return ENOEXEC;
	// }
...
load_elf(struct vnode *v, vaddr_t *entrypoint)
{
	...
	#ifdef _DB_MODE_
	if (_DB_MODE_) {	
		kprintf("load_elf: Before define segments loop. \n");
	}
	#endif
	...
	#ifdef _DB_MODE_
	if (_DB_MODE_) {	
		kprintf("load_elf: Before as_prepare_load. \n");
	}
	#endif
	...
	#ifdef _DB_MODE_
	if (_DB_MODE_) {	
		kprintf("load_elf: Before load segments loop. \n");
	}
	#endif
