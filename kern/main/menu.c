...
common_prog(int nargs, char **args)
{
	...
	// Well, I used fork here, but actually it is not a fork.
	// So vmspace should be a brand new one.
	struct addrspace *as = proc->p_vmspace;
	proc->p_vmspace = as_create();
	if(proc->p_vmspace == NULL){
		return ENOMEM;
	}
	as_destroy(as);

	pid_t progproc_id = proc->p_id; 
	...
	pid_t waitpidret = PID_MAX+1;
	int status = 5; // whatever

	// wait pid here

	while(progproc_id != waitpidret){
		sys_waitpid(progproc_id, &status, 0, &waitpidret);
		thread_yield();
	};
	#ifdef _DB_MODE_
	if (_DB_MODE_) {	
		kprintf("<boot/menu> get status from proc %d, exitcode: %d\n", progproc_id, status);
	}
	#endif
