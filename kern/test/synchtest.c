...
semtest(int nargs, char **args)
{
	...
	for (i=0; i<NTHREADS; i++) {
		result = thread_fork("semtest", NULL, NULL, i, semtestthread, NULL);
		if (result) {
...
locktest(int nargs, char **args)
{
	...
	for (i=0; i<NTHREADS; i++) {
		result = thread_fork("synchtest", NULL, NULL, i, locktestthread,
				     NULL);
		if (result) {
...
cvtest(int nargs, char **args)
{
	...
	for (i=0; i<NTHREADS; i++) {
		result = thread_fork("synchtest", NULL, NULL, i, cvtestthread,
				      NULL);
		if (result) {
