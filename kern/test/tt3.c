...
make_sleepalots(int howmany)
{
	...
	for (i=0; i<howmany; i++) {
		...
		result = thread_fork(name, NULL, NULL, i, sleepalot_thread, NULL);
		if (result) {
			panic("thread_fork failed: %s\n", strerror(result));
		}
	}
	result = thread_fork("waker", NULL, NULL, 0, waker_thread, NULL);
	if (result) {
		panic("thread_fork failed: %s\n", strerror(result));
	}
}
...
make_computes(int howmany)
{
	...
	for (i=0; i<howmany; i++) {
		...
		result = thread_fork(name, NULL, NULL, i, compute_thread, NULL);
		if (result) {
			panic("thread_fork failed: %s\n", strerror(result));
		}
	}
