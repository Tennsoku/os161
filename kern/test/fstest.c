...
doreadstress(const char *filesys)
{
	...
	for (i=0; i<NTHREADS; i++) {
		err = thread_fork("readstress", NULL, (void *)filesys, i, 
...
dowritestress(const char *filesys)
{
	...
	for (i=0; i<NTHREADS; i++) {
		err = thread_fork("writestress", NULL, (void *)filesys, i, 
...
dowritestress2(const char *filesys)
{
	...
	for (i=0; i<NTHREADS; i++) {
		err = thread_fork("writestress2", NULL, (void *)filesys, i, 
...
docreatestress(const char *filesys)
{
	...
	for (i=0; i<NTHREADS; i++) {
		err = thread_fork("createstress", NULL, (void *)filesys, i, 
